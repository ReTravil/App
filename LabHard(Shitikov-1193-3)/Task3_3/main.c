...
static	int32_t a = 85;																				//	Начальное значение члена арифметической прогрессии
static	int32_t d = 114;																      //	Переменная коэфициента для арифметической прогрессии
static	int32_t sum = 0;																			//	Начальное значение суммы
...
{																															//
	...
	while(1){																										//
		for (int32_t i = 1; i <=16; i++){													//	Счетчик для итераций
			counter = i;																						//	Приравнивание глобальной переменной к локальной перменной цикла
			a = Arif(a, d, i);																      //	Выполнение одной итерации арифметической прогрессии
			sum = sum + a;																					//	Сложение членов прогрессии 
			debug();																								//	Вызов функции отладчика
			if (i == 16){																						//	Когда цикл закончивается, значения для текущего члена и сумма членов прогрессии устанавливаются в значения по умолчанию
				a = 85;																								//
				sum = 0;																							//
			}																												//
		}																													//
...
void debug(){																									//	Функция отладчик
	while(1){																										//
		if(flag == 1){																						//	Флаг в значение 1 обозначает что была нажата клавиша F5																								
			while ((USART1->ISR & USART_ISR_TXE) == 0) {} 					//	Переход на новую строку со сдвигом каретки 
			USART1->TDR = 0x0D;																			// Новая строка
			while ((USART1->ISR & USART_ISR_TXE) == 0) {} 					//	
			USART1->TDR = 0x0A;                                     //	Каретка
      flag = 0;																			          //
			break;																									//	Выход из цикла = завершение функции
		}																													//
		if(flag == 2){																						//	Флаг в значение 2 обозначает что была нажата клавиша i
			while ((USART1->ISR & USART_ISR_TXE) == 0) {} 					//	
			USART1->TDR = 0x6E;																			// Это n 
			while ((USART1->ISR & USART_ISR_TXE) == 0) {} 					//	Вывод сообщения "n=номер_итерации"
			USART1->TDR = 0x3D;																			//	Это равно
			while ((USART1->ISR & USART_ISR_TXE) == 0) {} 					//	
			numToArray(counter);																		//
			flag = 0;																								//	Флаг нажатой кнопки принимает стандартное значение 0(обнуляем) 
		}																													//
		if(flag == 3){													    							//	Флаг в значение 3 обозначает что была нажата клавиша e
			while ((USART1->ISR & USART_ISR_TXE) == 0) {} 					//	
			USART1->TDR = 0x41;																			//	Это А
			while ((USART1->ISR & USART_ISR_TXE) == 0) {} 					//	
			USART1->TDR = 0x6E;																			//	Вывод сообщения "an=%текущее_значение_переменной%"
			while ((USART1->ISR & USART_ISR_TXE) == 0) {} 					//	
			USART1->TDR = 0x3D;																			//	Это s	
			while ((USART1->ISR & USART_ISR_TXE) == 0) {} 					//	
			numToArray(a);																					//
			flag = 0;																								//	Флаг нажатой кнопки принимает стандартное значение 0(обнуляем)
		}																													//
		if(flag == 4){																						//	Флаг в значение 4 обозначает что была нажата клавиша s
			while ((USART1->ISR & USART_ISR_TXE) == 0) {} 					//	
			USART1->TDR = 0x73;																			//
...
{																															//
	...
	if (USART1->ISR & USART_ISR_TC) {														//	Если в регистре состояний USART1 установлен флаг "ТС", то
		...
		USART1->ICR=USART_ICR_TCCF;																//	Сбросить флаг завершения передачи кадра, чтобы прерывание не сработало повторно
																															...
																															//
																															//
int32_t Arif(int32_t an, int32_t d, int32_t n){								//	Функция расчета арифметической прогресии 
	if (n == 1){																								//	Если итерация первая
		return an;																								//		возвращается первый член прогрессии
	}																														//
	else{																												//	Иначе
		return an+d;																							//		просиходит сложение переменных
	}																														//
}																															//
																															//
void numToArray(int32_t num){																	//	Функция вывода числа в терминал PuTTY
	...
	while (num!=0){																							//	Пока число не равное 0
		if (num > n){																							//	Получаем размер числа, пока num больше числа
			...
			while ((USART1->ISR & USART_ISR_TXE) == 0) {}						//	
			USART1->TDR = ((num-temp)/n) + 48;											//	В терминал подается пакет, соответвующий цифре текущего разряда. 
...
**																						's' для вывода суммы всех членов прогрессии;

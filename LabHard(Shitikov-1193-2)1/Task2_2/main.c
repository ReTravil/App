...
**Назначение программы: принятие по USART пакетов данных от ПК, запустить сведтодиоды и вернуть данные о их состоянии
...
int main(void)
{
	...
	GPIOB->MODER |= GPIO_MODER_MODER0_0 | GPIO_MODER_MODER1_0 | GPIO_MODER_MODER2_0 | 	//Переключение линий 0-8 порта B в режим "Output"
							    GPIO_MODER_MODER7_0 | GPIO_MODER_MODER6_0 | GPIO_MODER_MODER5_0 | 
							    GPIO_MODER_MODER4_0	| GPIO_MODER_MODER3_0 | GPIO_MODER_MODER8_0;
//Объявление переменных
	unsigned reg[8] = {0x1, 0x2, 0x4, 0x80, 0x40, 0x20, 0x14, 0x8};  																											//Регистры для запуска светодиодов
	unsigned input [9][9] = {{0x31,0}, {0x32,1}, {0x33,2}, {0x34,3}, {0x35,4}, {0x36,5}, {0x37,6}, {0x38,7}, {0x30,8}};		//Двумерный массив (Входящее число в hex и его соответствие в данной программе)
...
//Функция ввыода сообщения (number - номер светодиода, state - его состояние (0 - отключен, 1 - включен))
void msg (uint8_t number, unsigned state){
	uint8_t textLightdiod [10] = {0x6C, 0x69, 0x67, 0x68, 0x74, 0x64, 0x69, 0x6F, 0x64, 0x20};				//Массив содержащий сообщение "Lightdiod "
...
//Вывод сообщения "Lightdiod "
		for (uint16_t i=0; i<10; i++){
			...
			USART1->TDR = textLightdiod[i];													//Передача данных из програмной переменной в USART(регистр TDR)
